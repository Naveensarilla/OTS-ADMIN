...
const Testcreation = () => {
  ...
  const [testData, setTestData] = useState([]);
  const [selectedInstruction, setSelectedInstruction] = useState("");
  const [selectedInstructionId, setSelectedInstructionId] = useState(""); // New state
  const [instructionsData, setInstructionsData] = useState([]);
  ...
  const handleInstructionChange = (e) => {
    ...
    console.log("Selected Instruction ID:", selectedId);
    setSelectedInstructionId(selectedId);
    const selectedInstructionObj = instructionsData.find(
      (instruction) => instruction.instructionId === selectedId
    );
    setSelectedInstruction(selectedInstructionObj?.instructionHeading || "");
  };
  ...
  try {
    ...
    const response = await fetch("http://localhost:3081/create-test", {
      ...
      body: JSON.stringify({
        ...
        status,
        sectionsData,
        selectedInstructionId, // Use selectedInstructionId here
      }),
    });
